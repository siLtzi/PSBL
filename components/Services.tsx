...
export default function Services({
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-6 gap-[3px]">
        {services.map((service, index) => {
          const count = services.length;
          // For 5 items: first 3 take 2/6 cols, last 2 take 3/6 cols → symmetric two rows
          // For 4 items: 2+2, for 6 items: 3+3 (default 2 cols each)
          const colSpan =
            count === 5
              ? index < 3
                ? "xl:col-span-2"
              ...
              : "xl:col-span-2";
          const aspectClass =
            count === 5 && index >= 3 ? "aspect-square xl:aspect-[16/9]" : "aspect-square";

          const inner = (
            <div className={`relative overflow-hidden ${aspectClass} bg-[var(--black)] cursor-pointer group`}>
              <Image
          ...
          return service.ctaHref ? (
            <Link key={service.title} href={service.ctaHref} className={`block ${colSpan}`}>
              {inner}
            </Link>
          ) : (
            <div key={service.title} className={colSpan}>{inner}</div>
          );
        })}
      </div>
    </section>
